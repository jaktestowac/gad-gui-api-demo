/**
 * GadTalk Explore Page JavaScript
 * Handles search, trending hashtags, and discovery features
 */
...
(function () {
  ...
  const searchButton = document.getElementById("explore-search-btn");
  const trendingList = document.getElementById("trending-list");
  const exploreGadsList = document.getElementById("explore-gads-list");
  ...
  async function init() {
    ...
    setupEventListeners();
    await Promise.all([loadTrendingHashtags(), loadExploreFeed()]);
    setupInfiniteScroll();
  }
  ...
  function setupEventListeners() {
    ...
    // URL hash changes for hashtag navigation
  ...
  /**
   * Handle hash change for hashtag navigation
   */
  function handleHashChange() {
    const hash = window.location.hash.slice(1);
    if (hash && hash.startsWith("tag=")) {
      const hashtag = decodeURIComponent(hash.slice(4));
  ...
  async function loadTrendingHashtags() {
    ...
    try {
      const response = await GadTalkAPI.hashtags.getTrending(10);

      if (response.success && response.hashtags?.length > 0) {
        renderTrendingHashtags(response.hashtags);
      } else {
  ...
  function renderTrendingHashtags(hashtags) {
    ...
    // Add click handlers
    trendingList.querySelectorAll(".gt-hashtag-item").forEach((item) => {
      item.addEventListener("click", (e) => {
        e.preventDefault();
        const hashtag = item.dataset.hashtag;
        loadHashtagGads(hashtag);
        window.location.hash = `tag=${encodeURIComponent(hashtag)}`;
      });
    });
  }

  /**
   * Load explore feed (latest gads)
   */
  async function loadExploreFeed() {
    if (!exploreGadsList) return;
    ...
    if (exploreTitle) {
      exploreTitle.textContent = "Latest Gads";
    }

    GadTalkUI.showSkeletons(exploreGadsList, "gad", 5);

    try {
      const response = await GadTalkAPI.gads.getFeed(currentPage, ITEMS_PER_PAGE);

      if (response.success && response.gads?.length > 0) {
        renderGads(response.gads, false);
        hasMore = response.gads.length === ITEMS_PER_PAGE;
      } else {
        exploreGadsList.innerHTML = `
          <div class="gt-empty-state">
  ...
  /**
   * Load gads for a specific hashtag
   */
  async function loadHashtagGads(hashtag) {
    ...
    if (exploreTitle) {
      exploreTitle.textContent = `#${hashtag}`;
    }

    // Update active state in trending list
    trendingList?.querySelectorAll(".gt-hashtag-item").forEach((item) => {
      item.classList.toggle("active", item.dataset.hashtag === hashtag);
    });

    GadTalkUI.showSkeletons(exploreGadsList, "gad", 5);

    try {
      const response = await GadTalkAPI.hashtags.getGads(hashtag, currentPage, ITEMS_PER_PAGE);
      ...
      if (response.success && response.gads?.length > 0) {
        ...
        exploreGadsList.innerHTML = `
          <div class="gt-empty-state">
            ...
            <h3>No gads with #${hashtag}</h3>
            <p>Be the first to use this hashtag!</p>
            <button class="gt-btn gt-btn-secondary" onclick="window.GadTalkExplore.loadExploreFeed()">Back to explore</button>
          </div>
        `;
        hasMore = false;
      }
  ...
  async function loadMore() {
    ...
    exploreGadsList?.appendChild(loadingEl);

    try {
      let response;
      if (currentHashtag) {
        response = await GadTalkAPI.hashtags.getGads(currentHashtag, currentPage, ITEMS_PER_PAGE);
      } else {
        response = await GadTalkAPI.gads.getFeed(currentPage, ITEMS_PER_PAGE);
      }
  ...
  function renderGads(gads, append = false) {
    ...
    if (append) {
      exploreGadsList.insertAdjacentHTML("beforeend", html);
    } else {
      exploreGadsList.innerHTML = html;
    }
  ...
  /**
   * Setup gad interactions (like, regad, etc.)
   */
  ...
  function setupInfiniteScroll() {
    ...
    exploreGadsList?.parentElement?.appendChild(sentinel);
