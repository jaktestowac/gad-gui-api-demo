...
class RandomWeatherContext {
    ...
    generateWeatherData(days, city) {
        const limitedDays = Math.min(days || 7, 90);
        let weatherData = generateWeatherDataForNPastDays(limitedDays, true);
...
const handleGetWeather = (context, ws, data) => {
    const weatherData = context.generateWeatherData(data.days, data.city);
